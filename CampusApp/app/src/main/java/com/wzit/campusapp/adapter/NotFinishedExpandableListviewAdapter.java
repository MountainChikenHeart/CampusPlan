 ...
 * 任务适配器
 */
public class NotFinishedExpandableListviewAdapter extends BaseExpandableListAdapter {
    ...
    public NotFinishedExpandableListviewAdapter(Context context, List<String> list1, List<List<NewPlanBean>> list3){
    ...
    public View getChildView(int groupPosition, int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
        ...
        if (bean.getRank() == 0){
             ...
             childViewHolder.degree1.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                 @Override
                 public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                     if (planCheckInterface != null){
                         planCheckInterface.IsRedCheck(compoundButton,bean,b);
                     }
                 }
             });
        }else if (bean.getRank() == 1){
             childViewHolder.degree2.setVisibility(View.VISIBLE);
            childViewHolder.degree2.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                @Override
                public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                    if (planCheckInterface != null){
                        planCheckInterface.IsYellowCheck(compoundButton,bean,b);
                    }
                }
            });
        }if (bean.getRank() == 2){
            childViewHolder.degree3.setVisibility(View.VISIBLE);
            childViewHolder.degree3.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                @Override
                public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                    if (planCheckInterface != null){
                        planCheckInterface.IsBlueCheck(compoundButton,bean,b);
                    }
                }
            });
        }
